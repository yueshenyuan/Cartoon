...
// set originLayer's reflection bounds
    ...
    reflectLayer.position = CGPointMake(layer.bounds.size.width/2, layer.bounds.size.height*1.22);
...
- (void)moveOneStep:(BOOL)isSwipingToLeftDirection {
    ...
    [_delegate ClickCurrentImageIndex:self.currentRenderingImageIndex];
}
...
//        NSLog(@"lastIndex----%d",self.currentRenderingImageIndex);
    }
}

+ (id)coverFlowViewWithFrame:(CGRect)frame andImages:(NSMutableArray *)rawImages sideImageCount:(int)sideCount sideImageScale:(CGFloat)sideImageScale middleImageScale:(CGFloat)middleImageScale currentImgIndex:(int)CurrentIndex {
    ...
    flowView.currentRenderingImageIndex = CurrentIndex;
...
- (void)showImageAndReflection:(CALayer*)layer
{
    ...
    reflectLayer.position = CGPointMake(layer.bounds.size.width/2, layer.bounds.size.height*1.22);
    reflectLayer.bounds = CGRectMake(layer.bounds.origin.x, layer.bounds.origin.y, layer.bounds.size.width, layer.bounds.size.height/2);
